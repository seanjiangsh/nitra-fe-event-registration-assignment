...
<template>
  <div class="flex flex-col gap-1.5">
    <label :for="id" class="text-md font-medium" :class="hasError ? 'text-danger-emphasis' : 'text-neutral'">
    ...
    <input
      ...
      class="w-full px-3 py-2 rounded-lg text-md text-neutral bg-surface-l0 border border-solid transition-colors placeholder:text-neutral-quiet focus:outline-none"
      :class="
        hasError
      ...
      "
    />
    <p v-if="hasError" :id="`${id}-error`" class="text-sm text-danger-emphasis">{{ error }}</p>
