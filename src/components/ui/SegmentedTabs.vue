...
<template>
  <div role="tablist" :aria-label="ariaLabel" class="relative inline-flex w-fit gap-1 p-1 rounded-xl bg-surface-l2">
    ...
    <button
      ...
      class="relative z-1 appearance-none border-0 bg-transparent cursor-pointer px-5 py-2 rounded-lg text-md font-semibold transition-colors"
      :class="opt.id === modelValue ? 'text-inverse' : 'text-neutral-muted hover:text-neutral'"
      @click="select(opt.id)"
    >
      {{ opt.label }}
    </button>
