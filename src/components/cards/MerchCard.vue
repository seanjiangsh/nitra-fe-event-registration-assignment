...
<template>
  <BaseCard :selected="qty > 0" :interactive="false">
    <div class="flex items-start justify-between gap-3">
      <span class="text-subtitle1 text-neutral">{{ item.name }}</span>
      <span class="text-subtitle1 text-brand-emphasis shrink-0">{{ format(item.price) }}</span>
    </div>

    <p class="text-md text-neutral-muted m-0">{{ item.description }}</p>

    <!-- Controls: size (sized items) + qty stepper, inline -->
    <div class="flex flex-wrap items-center gap-x-5 gap-y-2">
      <div v-if="item.sizes" class="flex items-center gap-2">
        <label :for="`size-${item.id}`" class="text-sm text-neutral-muted">Size:</label>
        <select
          :id="`size-${item.id}`"
          :value="size"
      ...
      <div class="flex items-center gap-2">
        ...
        <button
          type="button"
          class="flex items-center justify-center w-8 h-8 rounded-md bg-surface-l2 text-neutral transition-colors hover:bg-neutral-muted-rest disabled:opacity-40 disabled:cursor-not-allowed"
          :disabled="qty <= 0"
        ...
        <span class="min-w-6 text-center text-md font-semibold text-neutral">{{ qty }}</span>
        <button
          type="button"
          class="flex items-center justify-center w-8 h-8 rounded-md bg-surface-l2 text-neutral transition-colors hover:bg-neutral-muted-rest disabled:opacity-40 disabled:cursor-not-allowed"
        ...
        <span class="text-sm text-neutral-quiet">max {{ item.maxQuantity }}</span>
      </div>
    </div>
