...
import { formatDayShort, formatTimeRange } from '../../composables/useDateTime.js'
...
const when = computed(() => `${formatDayShort(props.workshop.date)} · ${formatTimeRange(props.workshop.date, props.workshop.endDate)}`)
// Capacity counts the attendee's own reservation, like sessions.
const reserved = computed(() => props.workshop.registered + (props.selected ? 1 : 0))
const spotsRemaining = computed(() => Math.max(0, props.workshop.capacity - reserved.value))
...
<template>
  <BaseCard :selected="selected" :disabled="disabled" role="checkbox" @select="$emit('toggle')">
    <div class="flex items-start justify-between gap-3">
      ...
      <span class="text-subtitle1 text-brand-emphasis shrink-0">{{ format(workshop.price) }}</span>
    </div>

    <p class="text-md text-neutral-muted m-0">{{ workshop.description }}</p>
    ...
    <span v-if="conflictNote" class="text-sm font-medium text-danger-emphasis">{{ conflictNote }}</span>
  </BaseCard>
</template>
